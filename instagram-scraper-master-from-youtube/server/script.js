...
const script = async (username) => {
  ...
  await page.type("input[name=username]", "rendomzz22", { delay: 20 });
  await page.type("input[name=password]", "neelansh@123", { delay: 20 });
  ...
  const list1 = await page.$$(
    'div[role="dialog"] > div:nth-child(2) > ul > div > li > div > div > div:nth-child(2) > div > a'
  );
  let avatarPaths = [
  ...
  const list2 = await page.$$(
    'div[role="dialog"] > div:nth-child(3) > ul > div > li > div > div > div:nth-child(2) > div > a'
  );
  ...
  const notFollowingYou = following.filter(
    (item) => !followers.find((f) => f.username === item.username)
  );
  const notFollowingThem = followers.filter(
    (item) => !following.find((f) => f.username === item.username)
  );
  await browser.close();
  return {
    followerCnt,
